...
Disco.prototype._startRepl = function _startRepl() {
  ...
  .listen(this.config.replPort, this.config.replHost)
}
...
Disco.prototype._bind = function _bind(cb) {
  ...
  function ready() {
    ...
    debug('server ' + self.id + ' listening ' + address.address + ":" + address.port)
...
Disco.prototype._receiveMessage = function _receiveMessage(msg) {
  ...
  if (!event) {
    debug('Unrecognized event in message:', str(msg.meta()))
  } else {
