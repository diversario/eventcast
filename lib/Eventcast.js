...
Eventcast.prototype.logLevel = function logLevel(component, level) {
  ...
  if (typeof level == 'string' && this.log._logger[level]) {
    ...
    this.log.eventcast.warn('Unknown log level "' + level + '".')
  }
}
 ...
 * @param handler
 */
Eventcast.prototype.set = function (evtObject, payload, handler) {
 ...
 * @param event
 */
Eventcast.prototype.unset = function (event) {
  event.unschedule()
...
//  
//  if (!event) {
//    this.log.eventcast.info('Unrecognized event in message ' + msg.meta().name)
//  } else {
...
Eventcast.prototype._compatibilityCheck = function _compatibilityCheck(msg) {
  ...
  if (header.version !== Eventcast.protocolVersion) {
    error = new Error('Protocol version mismatch: expected ' + 
                        Eventcast.protocolVersion + ', got ' + header.version)
  }
