...
Eventcast.prototype._receiveMessage = function _receiveMessage(msg, rinfo) {
  ...
  if (msg.isMultipart()) {
    if (!msg.isComplete()) {
      return this.Message.buffer(msg)
    } else {
      this.Message.buffer(msg)
      message = new this.Message.IncomingMessage(this.Message.getBuffered(msg))
    }
  } else {
    message = msg
  }
...
Eventcast.prototype._receiveMultipartMessage = function _receiveMultipartMessage(msg) {
  this._messageBuffer[msg.meta().seqId] = this._messageBuffer[msg.meta().seqId] || []
  this._messageBuffer[msg.meta().seqId].push(msg)
}
