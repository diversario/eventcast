...
function MessageFactory(parent) {
  ...
  IncomingMessage.prototype.header = function header() {
    ...
    this._header = {
      version: parseInt(rawHeader.slice(frame.HEADER.version.START, frame.HEADER.version.START + frame.HEADER.version.LENGTH).toString(), 10),
      encrypted: !!parseInt(rawHeader.slice(frame.HEADER.encrypted.START, frame.HEADER.encrypted.START + frame.HEADER.encrypted.LENGTH).toString(), 10),
      metaLength: parseInt(rawHeader.slice(frame.HEADER.metaLength.START, frame.HEADER.metaLength.START + frame.HEADER.metaLength.LENGTH), 10)
    }
