...
function MessageFactory(parent) {
  ...
  OutgoingMessage.prototype.toChunks = function toChunks(maxSize) {
    var chunks = []
    ...
    while (pointer < message.length) {
      ...
      var length = om.sizeBeforePayload()
      om.setPayload(message.slice(pointer, pointer + maxSize - length))
      chunks.push(om.toBuffer())
      pointer = pointer + maxSize - length
    }
