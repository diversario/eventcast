...
//                 head.metaLength
  ...
  OutgoingMessage.prototype.header = function header(params) {
    ...
    var version = padString(this.protocolVersion, frame.VERSION.LENGTH)
      , encrypted = padString(!!this.config.encrypt | 0, frame.ENCRYPTED.LENGTH)
      , metaLength = padString(params.metaLength, frame.METALENGTH.LENGTH)
  ...
  IncomingMessage.prototype.header = function header() {
    ...
    var rawHeader = this.data.slice(frame.HEADER.START, frame.HEADER.LENGTH)

    this._header = {
      version: parseInt(rawHeader.slice(frame.VERSION.START, frame.VERSION.START + frame.VERSION.LENGTH).toString(), 10),
      encrypted: !!parseInt(rawHeader.slice(frame.ENCRYPTED.START, frame.ENCRYPTED.START + frame.ENCRYPTED.LENGTH).toString(), 10),
      metaLength: parseInt(rawHeader.slice(frame.METALENGTH.START, frame.METALENGTH.START + frame.METALENGTH.LENGTH), 10)
    }
