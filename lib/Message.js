...
function MessageFactory(parent) {
  ...
  function buffer(msg) {
    messageBuffer[msg.meta().seqId] = messageBuffer[msg.meta().seqId] || {'_messageBuffer': true}
    if (!messageBuffer[msg.meta().seqId][msg.meta().seq.toString()]) {
      messageBuffer[msg.meta().seqId][msg.meta().seq.toString()] = msg
    }
  }
  
  function getBuffer(msg) {
    var m = messageBuffer[msg.meta().seqId]
    delete messageBuffer[msg.meta().seqId]
    return m
  }
  ...
  IncomingMessage.prototype.assembleMultipartMessage = function (messages) {
    ...
    var meta = JSON.parse(reassembledMessage.slice(frame.HEADER.END, frame.HEADER.END + header.readInt16BE(0)))
