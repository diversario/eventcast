...
function MessageBuffer(config) {
  this._timeout = config && config.messageTtl || 3000
  this._timers = {
    receive: {},
    send: {},
    nack: {},
...
MessageBuffer.prototype.getSender = function getSender(seqId) {
  var parts = Object.keys(this._incomingMessages[seqId])
  
  return {
    host: this._incomingMessages[seqId][parts[0]].meta().host,
    port: this._incomingMessages[seqId][parts[0]].meta().port
  }
}
