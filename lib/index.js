...
Disco.prototype.attachDiscoveryProbe = function () {
  this.set({name: 'disco:announce', interval: 1000}, '', this.nodeOnline)
}
...
Disco.prototype.stop = function stop(cb) {
  ...
  Object.keys(self.events).forEach(self.unscheduleEvent.bind(self))
...
Disco.prototype.receiveMessage = function receiveMessage(msg) {
  if (this.events[msg.name].handler) {
    this.events[msg.name].handler.call(this, msg)
  } else {
...
Disco.prototype.dispatchMessage = function dispatchMessage(event) {
  ...
  var message = new this.Message.OutgoingMessage(event, this.events[event].payload).toBuffer()
